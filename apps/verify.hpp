...
// Every ray is intersected through the index and against every
// primitive, and the two distances are compared exactly.
inline bool the_index_changes_only_the_speed_of_the_answer() {
    ...
    //    it declines into must still reach every one of them.
    {
        ...
        held = held && reachable == pile.surfaces().size();
    }
...
inline bool radiance_does_not_depend_on_how_big_the_room_is() {
    using namespace render;
    bool held = true;
...
inline bool the_thread_count_changes_the_speed_and_not_the_answer() {
    using namespace render;
    bool held = true;
...
inline bool every_sampler_agrees_with_the_density_it_claims() {
    using namespace render;
    bool held = true;
    ...
    // One row, not three.
    //
    // This printed three — one per alternative of `Bsdf` — and they came out
    // to the same p-value to the last digit, because the three share
    // Lambert's sampling routine and were given the same seeds. Three copies
    // of one measurement, presented as three pieces of evidence, which is
    // exactly the kind of padding an inspection sheet must not have.
    //
    // So the sampler is tested once at four angles, and the fact that made
    // the other two rows redundant is asserted directly underneath, where it
    // is a stronger statement than three matching p-values ever were: the
    // alternatives do not merely agree, they draw the same direction bit for
    // bit. The day one stops sharing the routine, that row fails and a second
    // row is worth adding back.
    {
        double lowest = 1.0;
        long impossible = 0;
        int index = 0;
        ...
        for (const double degrees : {0.0, 30.0, 60.0, 85.0}) {
            ...
            const Result r = test(Dispatch{grey}, wo, draws,
        ...
        std::printf("  %-46s %9d draws   %s   worst p = %.3f\n",
                    "lambert's sampler, four angles", draws * 4,
...

    {
        ...
        for (int i = 0; i < draws; ++i) {
            ...
            const Vec3 a = sample(grey, wo, lambdas, u, v).wi;
            const Vec3 b = sample(spectral, wo, lambdas, u, v).wi;
            const Vec3 c = sample(measured, wo, lambdas, u, v).wi;

            if (a.x != b.x || a.y != b.y || a.z != b.z) ++differing;
            if (a.x != c.x || a.y != c.y || a.z != c.z) ++differing;
        }

        held = held && differing == 0;
        std::printf("  %-46s %9d draws   %s   %ld differ\n",
                    "and the three alternatives draw identically", draws,
                    differing == 0 ? "agree" : "DISAGREE", differing);
    }
...
inline bool lambert_and_a_mirror_vanish_in_the_furnace() {
    using namespace render;
    bool held = true;
...
inline bool every_density_integrates_to_one() {
    using namespace render;
    bool held = true;
...
inline bool every_bsdf_returns_the_same_value_swapped() {
    using namespace render;
    bool held = true;
...
inline bool no_non_finite_value_reaches_the_film() {
    using namespace render;
    bool held = true;
...
inline bool no_instrument_catches_everything() {
    using namespace render;
    bool held = true;
...
inline bool fresnel_and_the_things_that_fall_out_of_it() {
    using namespace render;
    bool held = true;
...
inline bool a_metals_colour_comes_out_of_its_table() {
    using namespace render;
    bool held = true;
    ...
    const auto check_metal = [&](const Row& row, const auto& index) {
        ...
        held = held && coloured_right && edge_right;
        (void)slope;
...
// ── The sheet ────────────────────────────────────────────────────────────
//
// Each section above is one claim. It prints itself and says whether it held,
// and this function is the contents page — the only part of the file that has
// to be read to know what the project checks.
//
// It was one function of fourteen hundred lines until it was not. Item 0153
// concluded, about `transport.hpp`, that length is never the test and one
// idea per file is. That is still true of this file, which is one idea. It
// was not true of the *function*: a reader could not see what the sheet
// checked without scrolling past every check, and two sections had drifted so
// far apart that the comment introducing the threading check sat above a
// block which opened with the scale one.
//
// `sheet::x() && all` rather than `all && sheet::x()`, deliberately.
// Short-circuiting would stop at the first failure, and an inspection sheet
// that stops at the first problem is one that hides the second.
...
inline int verify() {
    ...
    bool all = true;
    all = sheet::the_index_changes_only_the_speed_of_the_answer()        && all;
    all = sheet::radiance_does_not_depend_on_how_big_the_room_is()       && all;
    all = sheet::the_thread_count_changes_the_speed_and_not_the_answer() && all;
    ...
    all = sheet::fresnel_and_the_things_that_fall_out_of_it()            && all;
    all = sheet::a_metals_colour_comes_out_of_its_table()                && all;

    // Item 0065 asked for this to run in under a minute or to grow a
    // `--quick` that says what it skipped. It runs in a couple of seconds, so
